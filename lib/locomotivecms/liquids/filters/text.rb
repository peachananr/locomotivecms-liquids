    ...
    module Filters
      module Text # :nodoc:
          ...
          if type_of_post == "things to do"
            if html.css(".product-summary.itinerary-summary:not(.day-to-day)").size == 1
              ...
              list.each_with_index do |i, index| 
                ...
                l_map_url = ""
                if info_box
                  # Use .xpath for the LI to be case-insensitive and ignore emojis/whitespace
                  location_li = info_box.at_xpath(".//li[contains(., 'Location')]")

                  if location_li
                    map_link = location_li.at_css("a")
                    if map_link
                      l_map_url = map_link['href']
                      # Strip out "[Map Link]" and any trailing/leading whitespace
                      l_location_text = map_link.text.gsub(/\[.*?\]/, "").strip
                    end
                  end
                end
        ...
        def pdf_render(input)
          ...
          if html.css('.product-summary.accommodation').size > 0
            html.css('.product-summary.accommodation a').each do |a|
              new_a = "<a class='btn btn-primary' style='display: block;background: linear-gradient(to bottom, #ffc300 0%, #ff9000 100%);color: #000;' href='#{a["href"]}' rel='#{a["rel"]}' target='#{a["target"]}'>#{a.at_css(".ps-price span").inner_html}</a>"
        ...
        def remove_placeholder_img(input)
          ...
          if html.css('.post-summary').size > 0
            html.at_css('.post-summary').before('<h3>Quick Summary</h3>')
          end
