        ...
        def add_blocks(input, p_limit = 5, mode = "old")
          ...
          p_tags.each_with_index do |p_tag,index|
            ...
            # If reach limit, close Block
              ...
              if p_tag.name == "h2" or p_tag.name == "h3" or p_tag.name == "h4" or p_tag.css(".lightbox-full").length > 0 or (!p_tag["class"].nil? and (p_tag["class"].include? "video-block" or p_tag["class"].include? "audio-block"))
                next
              end
