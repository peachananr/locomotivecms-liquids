      ...
      module Text # :nodoc:
        ...
        def about_metadata(input, title, desc, slug, location, type_of_post)
          ...
          if type_of_post == "things to do"
            ...
            if html.css(".post-summary.day-to-day").size == 1
              ...
              list.each_with_index do |i, index| 
                l_pos = index + 1
                l_name = "#{i.css("th,td")[0].text.strip} #{i.css("th,td")[1].text.sub(/\b\d+\.\s*/, '').strip}"
        ...
        def remove_placeholder_img(input)
            ...
            html.css('.product-summary.itinerary-summary').each do |i|
              if i.css(".editor-choice").size > 0
                ...
                if fix_required == "true" and  i.css(".editor-choice").size > 1
                  summary_table_html = "<div class=\"post-summary-wrapper\"><table class=\"post-summary things-to-do-summary\"><tbody>#{summary_table}</tbody></table></div>"
          ...
          if html.css('.accommodation-block:not(.dont-move)').size == 1
            ...
            h2 = el.at_css('h2')["id"]
            if html.css(".pin-img-tag a img").length > 0 and (html.at_css(".pin-img-tag a img")["src"].downcase.include? "things-to-do" or html.at_css(".pin-img-tag a img")["src"].downcase.include? "itinerary") and html.css("body h2[id*='things-to-do'] ~ h3:nth-of-type(2)").length > 0 and html.css(".product-summary.accommodation").length == 1   
          ...
          if html.css('.activity-block').size == 1
            ...
            if html.css(".pin-img-tag a img").length > 0 and (html.at_css(".pin-img-tag a img")["src"].downcase.include? "things-to-do" or html.at_css(".pin-img-tag a img")["src"].downcase.include? "itinerary")
